...
tcpdump_p = start_dump_pcap_qemubr0(server_ip, client_port, "COMPOSITE_7")
...
req_pkt = IP(dst=server_ip)/TCP(sport=client_port, dport=SERVER_PORT, flags='A', seq=client_ISN + 21, ack=server_ISN + 1)/Raw(load=DUMMY_REQ[20:30])
send(req_pkt)

time.sleep(0.5)

finack_pkt = IP(dst=server_ip)/TCP(sport=client_port, dport=SERVER_PORT, flags='FA', seq=client_ISN + 41, ack=server_ISN + 1)
send(finack_pkt)

time.sleep(0.5)

# data in closing states
data_pkt = IP(dst=server_ip)/TCP(sport=client_port, dport=SERVER_PORT, flags='A', seq=client_ISN + 31, ack=server_ISN - 12345678)/Raw(load='A' * 20)
send(data_pkt)
...
for i in range(1000):
    req_pkt = IP(dst=server_ip)/TCP(sport=client_port, dport=SERVER_PORT, flags='A', seq=client_ISN + 1 + i, ack=server_ISN + 1)/Raw(load='A')
    send(req_pkt)
    time.sleep(0.01)
