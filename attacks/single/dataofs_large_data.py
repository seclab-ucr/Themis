...
tcpdump_p = start_dump_pcap(SERVER_IP, client_port, "DATAOFS_LARGE_DATA")
...
req_pkt = IP(dst=SERVER_IP)/TCP(sport=client_port, dport=SERVER_PORT, flags='A', seq=client_ISN + 1, ack=server_ISN + 1, dataofs=15)/Raw(load=DUMMY_REQ[:10])
send(req_pkt)
