...
tcpdump_p = start_dump_pcap(SERVER_IP, client_port, "BAD_TIMESTAMP_RST")
...
rst_pkt = IP(dst=SERVER_IP)/TCP(sport=client_port, dport=SERVER_PORT, flags='R', seq=client_ISN + 1, options=[('Timestamp', (0, server_ts)), ('NOP', 0), ('NOP', 0)])
#rst_pkt.show()
send(rst_pkt)
...
req_pkt = IP(dst=SERVER_IP)/TCP(sport=client_port, dport=SERVER_PORT, flags='A', seq=client_ISN + 1, ack=server_ISN + 1, options=[('Timestamp', (3000, server_ts)), ('NOP', 0), ('NOP', 0)])/Raw(load=HTTP_REQ)
send(req_pkt)
