...
tcpdump_p = start_dump_pcap(SERVER_IP, client_port, "SEQ_BEFORE_ISN")
...
req_pkt = IP(dst=SERVER_IP)/TCP(sport=client_port, dport=SERVER_PORT, flags='A', seq=client_ISN + 1 - 10, ack=server_ISN + 1)/Raw(load='A' * 10 + HTTP_REQ)
send(req_pkt)
