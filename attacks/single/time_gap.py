...
tcpdump_p = start_dump_pcap(SERVER_IP, client_port, "TIME_GAP")
...
req_pkt = IP(dst=SERVER_IP)/TCP(sport=client_port, dport=SERVER_PORT, flags='A', seq=client_ISN + 1, ack=server_ISN + 1, options=[('Timestamp', (3000, server_ts)), ('NOP', 0), ('NOP', 0)])/Raw(load=HTTP_REQ[:20])
send(req_pkt)

time.sleep(0.5)

req_pkt = IP(dst=SERVER_IP)/TCP(sport=client_port, dport=SERVER_PORT, flags='A', seq=client_ISN + 21, ack=server_ISN + 1, options=[('Timestamp', (3000+0x80000000, server_ts)), ('NOP', 0), ('NOP', 0)])/Raw(load=HTTP_REQ[20:])
send(req_pkt)
