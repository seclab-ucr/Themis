...
tcpdump_p = start_dump_pcap(server_ip, client_port, "RST_RIGHTMOST_SACK")
...
rst_pkt = IP(dst=server_ip)/TCP(sport=client_port, dport=SERVER_PORT, flags='R', seq=client_ISN + 1 + len(HTTP_REQ))
send(rst_pkt)
