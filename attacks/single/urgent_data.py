...
tcpdump_p = start_dump_pcap(SERVER_IP, client_port, "URGENT_DATA")
...
req_pkt = IP(dst=SERVER_IP)/TCP(sport=client_port, dport=SERVER_PORT, flags='UA', seq=client_ISN + 21, ack=server_ISN + 1, urgptr=1)/Raw(load=HTTP_REQ[20:])
send(req_pkt)
