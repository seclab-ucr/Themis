...
tcpdump_p = start_dump_pcap(SERVER_IP, client_port, "SMALL_SEGMENTS")
...
req_pkt = IP(dst=SERVER_IP)/TCP(sport=client_port, dport=SERVER_PORT, flags='A', seq=client_ISN + 1, ack=server_ISN + 1)/Raw(load=HTTP_REQ[:4])
send(req_pkt)

time.sleep(0.5)

req_pkt = IP(dst=SERVER_IP)/TCP(sport=client_port, dport=SERVER_PORT, flags='A', seq=client_ISN + 5, ack=server_ISN + 1)/Raw(load=HTTP_REQ[4:])
send(req_pkt)
