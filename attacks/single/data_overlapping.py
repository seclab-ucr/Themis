...
tcpdump_p = start_dump_pcap(SERVER_IP, client_port, "DATA_OVERLAPPING")
...
ooo_data_pkt1 = IP(dst=SERVER_IP)/TCP(sport=client_port, dport=SERVER_PORT, flags='A', seq=client_ISN + 21, ack=server_ISN + 1)/Raw(load=DUMMY_REQ[20:30])
send(ooo_data_pkt1)

time.sleep(0.5)

ooo_data_pkt2 = IP(dst=SERVER_IP)/TCP(sport=client_port, dport=SERVER_PORT, flags='A', seq=client_ISN + 21, ack=server_ISN + 1)/Raw(load=HTTP_REQ[20:30])
send(ooo_data_pkt2)
